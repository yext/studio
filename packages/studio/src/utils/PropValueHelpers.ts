import { PropType, PropVal, PropValueKind, PropValueType, TypeGuards } from "@yext/studio-plugin";
...
export default class PropValueHelpers {
  ...
  static getDefaultPropVal(propType: PropType): PropVal {
    const defaultValue = propType.type === PropValueType.Object
      ? Object.fromEntries(Object.entries(propType.shape).map(([name, propMetadata]) =>
        [name, this.getDefaultPropVal(propMetadata)]
      ))
      : PropValueHelpers.getLiteralPropDefaultValue(propType.type);
    const propVal = {
