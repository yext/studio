import { PropValueType, PropValueKind, TypeGuards, TEMPLATE_STRING_EXPRESSION_REGEX, PropValues } from "@yext/studio-plugin"
  ...
  Object.keys(props).forEach(propName => {
    ...
    if (kind === PropValueKind.Expression) {
      if (TypeGuards.isTemplateString(value)) {
        transformedProps[propName] = getTemplateStringValue(value, valueType, expressionSources)
      } else {
        transformedProps[propName] = getExpressionValue(value, valueType, expressionSources) ?? value
      }
    } else {
...
function getTemplateStringValue(
  ...
  return templateStringWithoutBacktiks.replaceAll(TEMPLATE_STRING_EXPRESSION_REGEX, (...args) => {
    const expressionVal = getExpressionValue(args[1], propType, expressionSources)
...
function getExpressionValue(
  ...
  function getValueFromPath(path: string, parentPath: string)  {
    ...
    if (!sourceObject) {
      console.warn(`Invalid expression source type: ${parentPath}. Unable to extract the desired data from path: ${path}`)
      return null
    }
    ...
    if (TypeGuards.isValidPropValue(propVal)) {
      ...
      console.warn(`Invalid expression prop value: ${newPropValue}. The value extracted from the expression "${path}" does not match with the expected propType ${propType}.`)
      return null
    }
  }
