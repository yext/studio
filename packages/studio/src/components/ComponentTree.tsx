import { Classes, DndProvider, DragItem, DragLayerMonitorProps, DropOptions, getBackendOptions, MultiBackend, NodeModel, PlaceholderRenderParams, Tree } from '@minoru/react-dnd-treeview'
...
export default function ComponentTree() {
  const activePageState = useStudioStore(store => store.pages.getActivePageState())
  ...
  const tree = useMemo(() => {
    return activePageState.componentTree.map(c => {
      const commonData = {
        id: c.uuid,
        text: c.kind === ComponentStateKind.Fragment ? 'Fragment' : c.componentName,
        parent: c.parentUUID ?? ROOT_ID,
        data: c
      }
      if (c.kind === ComponentStateKind.Fragment || c.kind === ComponentStateKind.BuiltIn) {
        return {
          ...commonData,
          droppable: true
        }
  ...
  const handleDrop = useCallback((tree: NodeModel<ComponentState>[]) => {
...
function renderNode(node: NodeModel<ComponentState>) {
  return (
    <div>{node.text} {node.data?.uuid}</div>
  )
}
