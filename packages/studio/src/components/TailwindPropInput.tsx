...
export default function TailwindPropInput({ onChange, value, disabled }: Props) {
  ...
  const removeTailwindClass = useCallback((tailwindClass: string) => {
    setIsOpen(false);
    const tailwindClasses = new Set(splitValue(value))
  ...
  const pillContainerClass = classNames("flex flex-wrap items-center border border-gray-300 focus:border-indigo-500 rounded-lg pt-2 pb-1 pl-2 pr-2 w-full", {
    "opacity-50": !!disabled
  })
    ...
    <div className="relative" ref={ref}>
      <div className={pillContainerClass}>
        {splitValue(value).map(tailwindClass => {
          return (
            <TailwindClassPill key={tailwindClass} removeTailwindClass={removeTailwindClass} tailwindClass={tailwindClass}/>
          )
        })}
        {hasUnusedClasses && <EmbedIcon
        className='mb-0.5'
          role="button"
...
function TailwindClassPill(props: { tailwindClass: string, removeTailwindClass: (val: string) => void }) {
  ...
  return (
    <div className='mr-1 mb-1 flex bg-sky-100 rounded px-1 hover:bg-sky-200 items-center whitespace-nowrap hover:cursor-pointer' onClick={handleClick}>
      {tailwindClass}
      <X className='ml-1' />
    </div>
  )
...
function renderDropdown(safelist: string[], value: string, addTailwindClass: (val: string) => void) {
  return (
    <ul
      className="absolute w-max bg-white mt-2 rounded border shadow-2xl z-10 opacity-100"
    >
      {safelist.map((tailwindClass) => {
        return <SafelistItem key={tailwindClass} addTailwindClass={addTailwindClass} tailwindClass={tailwindClass} />
      })}
    </ul>
  )
}
