...
export const createPagesSlice: SliceCreator<PagesSlice> = (set, get) => {
  const pagesActions = {
    ...
    setActivePageState: (pageState) => set(store => {
  ...
  const activeComponentActions = {
    ...
    getActiveComponent: () => {
      ...
      return get().getActivePageState().componentTree.find(c => c.uuid === activeComponentUUID)
    },
    setActiveComponentProps: (props: PropValues) => set(store => {
      const activeComponent = get().getActiveComponent()
      if (!activeComponent) {
        console.error('Error in setActiveComponentProps: No active component selected in store.')
        return;
      }
