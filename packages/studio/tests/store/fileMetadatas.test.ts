import { ComponentMetadata, FileMetadataKind, PropValueType } from '@yext/studio-plugin'
...
describe('FileMetadatasSlice', () => {
  ...
  it('updates UUIDToFileMetadata using setFileMetadata', () => {
    useStudioStore.getState().fileMetadatas.setFileMetadata('some-uuid', componentMetadata)
    const UUIDToFileMetadata = useStudioStore.getState().fileMetadatas.UUIDToFileMetadata
  ...
  it('returns a FileMetadata using getFileMetadata', () => {
    ...
    const fileMetadata = useStudioStore.getState().fileMetadatas.getFileMetadata('uuid-1')
