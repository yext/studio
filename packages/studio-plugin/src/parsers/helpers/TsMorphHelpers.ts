...
export default class TsMorphHelpers {
  ...
  /**
   * Similar to ts-morph's getLastChildByKind but accepts multiple kinds.
   */
  private static getLastChildOfKind<T extends ReadonlyArray<SyntaxKind>>(
    node: Node,
    ...kinds: T
  ): KindToNodeMappings[OneOf<T>] | undefined {
    return node.getLastChild((n) => kinds.some((k) => n.isKind(k))) as
      | KindToNodeMappings[OneOf<T>]
      | undefined;
  }
  ...
  static getLastChildOfKindOrThrow<T extends ReadonlyArray<SyntaxKind>>(
    ...
    const foundNode = TsMorphHelpers.getLastChildOfKind(node, ...kinds);
    if (!foundNode) {
      throw new Error(
        `Could not find a child of kind ${kinds
          .map((k) => {
