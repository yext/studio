...
import { ComponentMetadata, FileMetadata, ModuleMetadata, PageState } from "../types";
import fs from "fs";
...
export default class ParsingOrchestrator {
  ...
  private getFileMetadata(absPath: string): FileMetadata {
    ...
    if (absPath.startsWith(this.paths.modules)) {
      const moduleFile = new ModuleFile(absPath, this.getFileMetadata, tsMorphProject);
      return moduleFile.getModuleMetadata();
    }
    const { modules, components } = this.paths;
    throw new Error(`Unknown filepath ${absPath}, expected ${modules} or ${components}.`);
  }
  ...
  private getPageNameToPageState(): Record<string, PageState> {
    ...
    return fs.readdirSync(this.paths.pages, "utf-8").reduce((prev, curr) => {
      ...
      const pageFile = new PageFile(path.join(this.paths.pages, curr), this.getFileMetadata, tsMorphProject);
